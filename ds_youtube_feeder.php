...
/* ACTIONS */
if ( is_admin() ){
	add_action( 'admin_menu', 'my_plugin_menu' );       // creates a new menu item under 'Settings'
	add_action( 'admin_init', 'register_mysettings' );  // register plugin settings
	add_option( 'ds-yt-user' );
}

/* Creates a new menu item under 'Settings' for Options Page */
function my_plugin_menu() {
	add_options_page( 'Youtube Feeder Options', 'Youtube Feeder', 'manage_options', 'ds-youtube-feeder', 'my_plugin_options' );
}
...
<p>This plugin accesses YouTube feeds using the following request:<br>
	...
	Where "USERNAME" is the <b>Youtube Username</b> supplied below, and "#" is the number of results to pull, as set in the shortcode.</p>
